...
exports.getPlayerFull = async (req, res) => {
  const playerId = req.params.id;

  try {
    const fullInfo = await playerModel.getPlayerFull(playerId);

    if (!fullInfo.success || !fullInfo.data) {
      return res.status(404).json({ message: "Full player info not found." });
    }

    res.status(200).json({
      message: "Full player info retrieved successfully.",
      player: fullInfo.data.player,
      stats: fullInfo.data.stats,
    });
  } catch (err) {
    res.status(500).json({
      message: "Internal server error during full player info retrieval.",
      error: err.message,
    });
  }
};
